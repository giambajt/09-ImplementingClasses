...
class Point(object):
    ...
    def move_by(self,dx,dy):
        self.dist = self.dist + math.sqrt(((self.x+dx) - self.x) ** 2 + ((self.y+dy) - self.y) ** 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def get_distance_from_start(self):
        x = self.x - self.firstx
        y = self.y - self.firsty
        return (math.sqrt((x**2) + (y**2)))
    ...
    def closer_to(self, Point1, Point2):
        if math.sqrt((Point1.x - self.x)**2 + (Point1.y - self.y)**2) <= math.sqrt((Point2.x - self.x)**2 + (Point2.y - self.y)**2):
            return Point1
        else:
            return Point2
